...
def read_url_romeo_and_juliet():
    response = requests.get('http://www.gutenberg.org/files/1112/1112.txt', timeout=10)
...
def cat_weight_stats(data):
    w = [r2i(c['weight']['metric']) for c in data if 'weight' in c]
    return min(w), max(w), mean(w), median(w), stdev(w)

def cat_life_stats(data):
    l = [r2i(c['life_span']) for c in data if 'life_span' in c]
    return min(l), max(l), mean(l), median(l), stdev(l)

from collections import Counter
def cat_country_breed_freq(data):
    return Counter(c["origin"] for c in data), Counter(c["name"] for c in d)
